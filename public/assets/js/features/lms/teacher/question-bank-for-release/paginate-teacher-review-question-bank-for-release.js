function paginateQuestionForReleaseDetail() {
    const container = document.getElementById('container-paginate-teacher-review-question-bank-for-release-list');
    const role = container.dataset.role;
    const schoolName = container.dataset.schoolName;
    const schoolId = container.dataset.schoolId;
    const assessmentQuestionId = container.dataset.assessmentQuestionId;

    if (!container) return;
    if (!role) return;
    if (!schoolName) return;
    if (!schoolId) return;
    if (!assessmentQuestionId) return;

    fetchBankSoalDetail(schoolName, schoolId, assessmentQuestionId);

    function fetchBankSoalDetail() {
        $.ajax({
            url: `/lms/${role}/${schoolName}/${schoolId}/teacher-question-bank-for-release/review/${assessmentQuestionId}/paginate`,
            ...
            success: function (response) {
                ...
                if (response.data.length > 0) {
                    ...
                    `;
                        ...
                        if (question.lms_question_bank?.tipe_soal !== 'MATCHING' && question.lms_question_bank?.tipe_soal !== 'ESSAY' && question.lms_question_bank?.tipe_soal !== 'PG_KOMPLEKS') {
                                matchingContainer = `
                                <div>
                        ...
                        if (question.lms_question_bank?.tipe_soal === 'PG_KOMPLEKS') {
                                    ...
                                    <table class="w-full border border-gray-300 text-sm">
                                        <thead>
                                            <tr class="bg-gray-100 text-center">
                                                <th class="border px-4 py-2">${question.lms_question_bank?.header_item ?? 'Pernyataan'}</th>
                        ...
                        } else if (question.lms_question_bank?.tipe_soal === 'MATCHING') {
                        ...
                        if (question.lms_question_bank?.tipe_soal === 'MATCHING') {
                    ...
                    $('.pagination-container-paginate-teacher-review-question-bank-for-release-list').html(response.links);
...
$(document).ready(function () {
    paginateQuestionForReleaseDetail();
});
