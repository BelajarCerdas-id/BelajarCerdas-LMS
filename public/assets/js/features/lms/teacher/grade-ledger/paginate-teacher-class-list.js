function teacherClassList(search_year = null, search_class = null, search_subject = null) {
    ...
    $.ajax({
        url: `/lms/${role}/${schoolName}/${schoolId}/grade-ledger/classes/paginate`,
        method: 'GET',
        data: {
        ...
        success: function (response) {
            ...
            if (response.data.length > 0) {
                $.each(response.data, function (index, item) {
                    const teacherGradeLedger = response.teacherGradeLedger.replace(':role', role).replace(':schoolName', schoolName).replace(':schoolId', schoolId).replace(':subjectTeacherId', item.id);
