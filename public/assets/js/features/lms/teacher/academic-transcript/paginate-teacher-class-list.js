function teacherClassList(search_year = null, search_class = null, search_subject = null) {
    ...
    $.ajax({
        url: `/lms/${role}/${schoolName}/${schoolId}/academic-transcript/classes/paginate`,
        method: 'GET',
        data: {
        ...
        success: function (response) {
            ...
            if (response.data.length > 0) {
                $.each(response.data, function (index, item) {
                    const teacherAcademicTranscript = response.teacherAcademicTranscript.replace(':role', role).replace(':schoolName', schoolName).replace(':schoolId', schoolId)
                        .replace(':subjectTeacherId', item.id);
                    ...
                    const card = `
                        <div class="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all duration-300">
                            ...
                            <!-- Action -->
                            <div class="mt-5">
                                <a href="${teacherAcademicTranscript}"
