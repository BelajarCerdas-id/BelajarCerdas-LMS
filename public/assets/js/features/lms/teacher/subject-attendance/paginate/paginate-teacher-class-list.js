function teacherClassList(search_year = null, search_class = null, search_subject = null) {
    ...
    $.ajax({
        url: `/lms/${role}/${schoolName}/${schoolId}/subject-attendance/classes/paginate`,
        method: 'GET',
        data: {
        ...
        success: function (response) {
            ...
            containerDropdownTahunAjaran.innerHTML = `
                <div class="flex flex-col w-full mb-2">
                    ...
                    <select id="dropdown-tahun-ajaran" class="w-full h-11 bg-white border border-gray-300 rounded-xl px-4 pr-10 text-sm font-medium text-gray-700 outline-none transition-all duration-200 hover:border-gray-400 focus:border-[#0071BC] focus:ring-2 focus:ring-[#0071BC]/20 cursor-pointer">
            ...
            if (response.data.length > 0) {
                $.each(response.data, function (index, item) {
                    const subjectAttendanceMeetingList = response.subjectAttendanceMeetingList.replace(':role', role).replace(':schoolName', schoolName)
                        .replace(':schoolId', schoolId).replace(':subjectTeacherId', item.id);
                    ...
                    const card = `
                        <div class="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all duration-300">
                            ...
                            <!-- Action -->
                            <div class="mt-5">
                                <a href="${subjectAttendanceMeetingList}"
