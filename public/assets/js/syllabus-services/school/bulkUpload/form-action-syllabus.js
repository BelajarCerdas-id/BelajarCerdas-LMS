...
$('#submit-button-bulkUpload-syllabus').on('click', function (e) {
    ...
    $.ajax({
        url: `/lms/${role}/school-subscription/${schoolName}/${schoolId}/academic-management/${curriculumName}/${curriculumId}/${faseId}/bulkupload-silabus/validate`,
        method: 'POST',
        headers: {
        ...
        contentType: false,
        success: function () {
            startUpload(formData, role, schoolName, schoolId, curriculumName, curriculumId, faseId, btn, defaultButtonHtml);
        },
        error: function (xhr) {
            if (xhr.status === 422) {
...
function startUpload(formData, role, schoolName, schoolId, curriculumName, curriculumId, faseId, btn, defaultButtonHtml) {
