function paginateSyllabusBab(page = 1) {
    const container = document.getElementById('container-management-bab');
    ...
    fetchFilteredDataSyllabusKelas(curriculumName, curriculumId, faseId, kelasId, mapelId);
    
    function fetchFilteredDataSyllabusKelas() {
        $.ajax({
            url: `/paginate-syllabus-service-bab/${curriculumName}/${curriculumId}/${faseId}/${kelasId}/${mapelId}`,
            method: 'GET',
            data: { page: page },
            success: function (data) {
                ...
                if (data.data.length > 0) {
                    ...
                    $.each(data.data, function (index, item) {
                        ...
                        const subBabDetail = data.subBabDetail.replace(':curriculumName', curriculumName).replace(':curriculumId', curriculumId).replace(':faseId', faseId)
                            .replace(':kelasId', kelasId).replace(':mapelId', mapelId).replace(':babId', item.id);
    
                        $('#tableListSyllabusBab').append(`
                            <tr class="text-xs">
                                <td class="border border-gray-300 px-3 py-2">${item.nama_bab ?? '-'}</td>
                                    ...
                                    <div class="dropdown dropdown-left">
                                        ...
                                        <ul tabindex="0"
                                            class="dropdown-content menu bg-base-100 rounded-box w-max p-2 shadow-sm z-9999">
                                                <li class="text-xs">
                                                    <a href="#" class="btn-edit-bab" data-curriculum-id="${curriculumId}" data-fase-id="${faseId}" data-kelas-id="${kelasId}"
                                                        data-mapel-id="${mapelId}" data-bab-id="${item.id}" data-bab-name="${item.nama_bab}" data-semester="${item.semester}">
...
function bindPaginationLinks() {
    $('.pagination-container-syllabus-bab').off('click', 'a').on('click', 'a', function (event) {
        ...
        paginateSyllabusBab(page); // Ambil data yang difilter untuk halaman yang ditentukan
    });
}
...
$('#submit-button-create-bab').on('click', function (e) {
    ...
    $.ajax({
        url: `/syllabus/${curriculumId}/${faseId}/${kelasId}/${mapelId}/bab/store`,
        method: 'POST',
        headers: {
...
$('#submit-button-edit-bab').on('click', function (e) {
    ...
    $.ajax({
        url: `/syllabus/curriculum/bab/edit/${curriculumId}/${faseId}/${kelasId}/${mapelId}/${babId}`,
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
...
$(document).ready(function () {
    $(document).on('change', '.toggle-bab', function () {
        ...
        $.ajax({
            url: '/syllabus/curriculum/bab/activate/' + babId, // Endpoint ke server
