...
<div class="relative left-0 md:left-62.5 min-h-screen w-full md:w-[calc(100%-250px)]">
    <div class="my-15 mx-7.5">
        <div class="bg-white rounded-xl shadow-md p-8 text-center">
            <div class="mb-6">
                @if($attempt->score >= $attempt->TkaExam->passing_score)
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                <div class="bg-blue-50 p-6 rounded-xl">
                    ...
                    <p class="text-4xl font-bold {{ $attempt->score >= 70 ? 'text-green-600' : 'text-orange-600' }}">
                        {{ $attempt->score }}
                    </p>
                </div>
            ...
            <div class="flex gap-4 justify-center">
                <a href="{{ route('tka-exams.index') }}" class="px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-semibold">
                    <i class="fa-solid fa-list mr-2"></i> Lihat TKA Lain
                </a>
                @if($attempt->score < $attempt->TkaExam->passing_score)
                    <a href="{{ route('tka-exams.start', $attempt->TkaExam->id) }}" class="px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 font-semibold">
