...
<div class="relative left-0 md:left-62.5 w-full md:w-[calc(100%-250px)] min-h-screen bg-gray-50">
    <div class="py-8 px-4 md:px-8">
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-bold text-gray-800 mb-2">Tambah Virtual Lab</h1>
        </div>

        <form action="{{ route('virtual-labs.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-3xl shadow-xl p-8">
            @csrf
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div class="space-y-6">
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Judul *</label>
                        <input type="text" name="title" required class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-purple-500">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Deskripsi *</label>
                        <textarea name="description" rows="4" required class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-purple-500"></textarea>
                    </div>
                    <div>
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <select name="kurikulum_id" required class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-purple-500">
                            ...
                            @foreach($kurikulums as $kur)<option value="{{ $kur->id }}">{{ $kur->nama_kurikulum }}</option>@endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="kelas_id" required class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-purple-500">
                            ...
                            @foreach($kelasList as $kelas)<option value="{{ $kelas->id }}">{{ $kelas->nama_kelas }}</option>@endforeach
                        </select>
                    </div>
                    <div>
                    ...
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Video URL (YouTube)</label>
                        <input type="text" name="video_url" placeholder="https://youtube.com/watch?v=..." class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-purple-500">
                    </div>
                    <div>
            ...
            <div class="flex gap-4 mt-8 pt-8 border-t">
                ...
                <button type="submit" class="flex-1 px-6 py-3 bg-gradient-to-r from-purple-500 to-purple-600 text-white rounded-xl font-bold shadow-lg">Simpan</button>
            </div>
        </form>
    </div>
</div>
