...
<div class="relative left-0 md:left-62.5 w-full md:w-[calc(100%-250px)] transition-all duration-500 ease-in-out z-20 min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
    <div class="py-8 px-4 md:px-8">
            ...
            <div class="relative z-10 px-6 py-8 md:px-10 md:py-10">
                <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                    <div class="flex items-start gap-4">
                        ...
                        <div>
                            <h1 class="text-3xl md:text-4xl font-extrabold text-white mb-2">Virtual Lab Management</h1>
                            <p class="text-blue-100 text-base">Kelola laboratorium virtual untuk pembelajaran</p>
                        </div>
                    </div>
                    <a href="{{ route('virtual-labs.create') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-white text-[#0071BC] rounded-xl hover:bg-blue-50 transition font-bold shadow-lg hover:shadow-xl hover:scale-105">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8">
                    <div class="bg-white/20 backdrop-blur-sm rounded-xl p-4 text-center border border-white/30">
                        <p class="text-3xl font-bold text-white">{{ $labs->total() }}</p>
                        <p class="text-sm text-blue-100">Total Labs</p>
                    </div>
                    <div class="bg-white/20 backdrop-blur-sm rounded-xl p-4 text-center border border-white/30">
                        <p class="text-3xl font-bold text-white">{{ $labs->lastPage() }}</p>
                        <p class="text-sm text-blue-100">Halaman</p>
                    </div>
                    <div class="bg-white/20 backdrop-blur-sm rounded-xl p-4 text-center border border-white/30">
                        <p class="text-3xl font-bold text-white">{{ $labs->perPage() }}</p>
        ...
        <div class="bg-white rounded-3xl shadow-xl p-6 mb-8 border border-gray-100">
            <form action="{{ route('virtual-labs.manage') }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">
                        <i class="fa-solid fa-magnifying-glass text-[#0071BC] mr-2"></i>Pencarian
                    </label>
                    <input type="text" name="search" value="{{ $search }}" placeholder="Cari virtual lab..." 
                ...
                <div>
                    ...
                    <select name="subject" onchange="this.form.submit()" 
                        ...
                        @foreach(['Fisika' => 'Fisika', 'Kimia' => 'Kimia', 'Biologi' => 'Biologi'] as $val => $lbl)
                            <option value="{{ $val }}" {{ $subject == $val ? 'selected' : '' }}>{{ $lbl }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <table class="w-full">
                    <thead class="bg-gradient-to-r from-[#0071BC] to-blue-600 text-white">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left font-bold">Views</th>
                    ...
                    <tbody>
                        @forelse($labs as $index => $lab)
                            <tr class="border-b hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 transition">
                                <td class="px-6 py-4 text-gray-600 font-semibold">{{ $labs->firstItem() + $index }}</td>
                                <td class="px-6 py-4">
                                    <div class="flex items-start gap-3">
                                        ...
                                        <div>
                                            <p class="font-bold text-gray-800">{{ $lab->title }}</p>
                                            <p class="text-sm text-gray-500 mt-1">{{ Str::limit($lab->description, 60) }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-gray-600 font-medium">{{ $lab->Kelas->nama_kelas ?? '-' }}</td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('virtual-labs.edit', $lab->id) }}" 
                                        ...
                                        <form action="{{ route('virtual-labs.delete', $lab->id) }}" method="POST" 
                                ...
                                <td colspan="6" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center">
                                        <div class="w-24 h-24 bg-gradient-to-br from-blue-100 to-indigo-100 rounded-full flex items-center justify-center mb-4">
                                            <i class="fa-solid fa-flask text-5xl text-[#0071BC]"></i>
                                        </div>
                                        <p class="text-gray-500 font-bold text-lg mb-2">Belum ada virtual lab</p>
                                        <p class="text-gray-400 text-sm mb-4">Buat virtual lab pertama untuk siswa</p>
                                        <a href="{{ route('virtual-labs.create') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-[#0071BC] to-blue-600 text-white rounded-xl hover:from-blue-600 hover:to-blue-700 transition font-bold shadow-lg">
                                            <i class="fa-solid fa-plus"></i>Tambah Lab Pertama
                                        </a>
                                    </div>
                                </td>
                            </tr>
