...
@if(in_array($book->tipe, ['buku','lks']) && $relatedBooks->count())
...
{{-- PDF VIEWER --}}
@if(in_array($book->tipe, ['buku','lks']))
...
<div class="pdf-header">
    ...
    <div style="display:flex; gap:10px;">
        <button onclick="prevPdfPage()" class="ppt-btn">◀</button>
        <button onclick="nextPdfPage()" class="ppt-btn">▶</button>
...
const fileType = "{{ in_array($book->tipe, ['ppt']) ? 'ppt' : 'pdf' }}";
...
async function loadPdf(){
    pdfDocGlobal = await pdfjsLib.getDocument(url).promise;
    pdfPageNum = 1;
...
async function renderPdfPage(num){
    ...
    const page = await pdfDocGlobal.getPage(num);
...
function nextPdfPage(){
    if(pdfPageNum >= pdfDocGlobal.numPages) return;
...
document.addEventListener("keydown", function(e){
    ...
    // PDF MODE
    if(document.getElementById("pdfViewer").style.display === "flex"){
        if(e.key === "ArrowDown") nextPdfPage();
        if(e.key === "ArrowUp") prevPdfPage();
    }
