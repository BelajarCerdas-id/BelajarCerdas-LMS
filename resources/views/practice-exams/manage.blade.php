...
<div class="relative left-0 md:left-62.5 w-full md:w-[calc(100%-250px)] transition-all duration-500 ease-in-out z-20 min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
    <div class="py-8 px-4 md:px-8">
            ...
            <div class="relative z-10 px-6 py-8 md:px-10 md:py-10">
                <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                    <div class="flex items-start gap-4">
                        <div class="p-4 bg-white/30 rounded-2xl backdrop-blur-sm shadow-lg">
                            <i class="fa-solid fa-pen-to-square text-4xl text-white"></i>
                        </div>
                        <div>
                            <h1 class="text-3xl md:text-4xl font-extrabold text-white mb-2">Latihan Soal Management</h1>
                            <p class="text-blue-100 text-base">Kelola koleksi latihan soal dan ujian</p>
                        </div>
                    </div>
                    <a href="{{ route('practice-exams.create') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-white text-[#0071BC] rounded-xl hover:bg-blue-50 transition font-bold shadow-lg hover:shadow-xl hover:scale-105">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8">
                    <div class="bg-white/20 backdrop-blur-sm rounded-xl p-4 text-center border border-white/30">
                        ...
                        <p class="text-sm text-blue-100">Total Latihan</p>
                    </div>
                    <div class="bg-white/20 backdrop-blur-sm rounded-xl p-4 text-center border border-white/30">
        ...
        <div class="bg-white rounded-3xl shadow-xl p-6 mb-8 border border-gray-100">
            <form action="{{ route('practice-exams.manage') }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">
                        <i class="fa-solid fa-magnifying-glass text-[#0071BC] mr-2"></i>Pencarian
                    </label>
                    <input type="text" name="search" value="{{ $search }}" placeholder="Cari latihan soal..." 
                ...
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">
                        <i class="fa-solid fa-filter text-[#0071BC] mr-2"></i>Tipe Ujian
                    </label>
                    <select name="type" onchange="this.form.submit()" 
                        ...
                        @foreach($examTypes as $value => $label)
                            <option value="{{ $value }}" {{ $examType == $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                    ...
                    <tbody>
                        @forelse($exams as $index => $exam)
                            <tr class="border-b hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 transition">
                                ...
                                <td class="px-6 py-4 text-gray-600 font-medium">{{ $exam->Kelas->nama_kelas ?? '-' }}</td>
                                <td class="px-6 py-4 text-gray-600 font-medium">{{ $exam->Mapel->nama_mapel ?? '-' }}</td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('practice-exams.edit', $exam->id) }}" 
                                        ...
                                        <form action="{{ route('practice-exams.delete', $exam->id) }}" method="POST" 
                                ...
                                <td colspan="7" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center">
                                        <div class="w-24 h-24 bg-gradient-to-br from-blue-100 to-indigo-100 rounded-full flex items-center justify-center mb-4">
                                            <i class="fa-solid fa-pen-to-square text-5xl text-[#0071BC]"></i>
                                        </div>
                                        <p class="text-gray-500 font-bold text-lg mb-2">Belum ada latihan soal</p>
                                        <p class="text-gray-400 text-sm mb-4">Buat latihan soal pertama untuk siswa</p>
                                        <a href="{{ route('practice-exams.create') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-[#0071BC] to-blue-600 text-white rounded-xl hover:from-blue-600 hover:to-blue-700 transition font-bold shadow-lg">
                                            <i class="fa-solid fa-plus"></i>Tambah Latihan Pertama
                                        </a>
                                    </div>
                                </td>
                            </tr>
